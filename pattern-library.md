...
| Prefix Sum | Multiple range-sum queries, overlapping ranges, contiguous subarray sums | prefix[i] stores the sum of the first i elements | Precompute cumulative sums, then answer [L, R] using prefix[R + 1] - prefix[L] | Build: O(n), Query: O(1) | O(n) |